...
# 后处理函数
def postprocess(output, image, ratio, x_offset, y_offset, confidence_threshold=0.2):
    COCO_CLASSES = ["fish"]  # 替换为实际的类别名称
...
class VideoProcessingWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # 默认初始化
        self.ip = '127.0.0.1'  # 默认 IP 地址
    ...
    def process_video(self):
        ...
        if ret:
            ...
            if self.session is not None:
                ...
                processed_frame = postprocess(output, frame, ratio, x_offset, y_offset, self.threshold)
